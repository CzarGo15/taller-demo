...
export default function App() {
  ...
  if (authError) return <div className="p-8 text-center text-red-600 font-bold">Error de Autenticación: {authError}</div>;
      ...
      {view === 'list' && (
        <div className="p-6 max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold text-blue-900 flex items-center gap-2"><Car className="w-8 h-8" /> Taller Arredondo</h1>
            <button onClick={createNewOrder} className="bg-blue-600 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-2 hover:bg-blue-700 transition"><Plus className="w-5 h-5" /> Nueva Orden</button>
          </div>
          <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
            {orders.length === 0 ? <div className="p-12 text-center text-gray-400">No hay órdenes registradas aún.</div> : 
              <div className="divide-y divide-gray-100">{orders.map((order: any) => (
                  <div key={order.id} className="p-4 hover:bg-blue-50 cursor-pointer transition flex justify-between items-center" onClick={() => { setCurrentOrder(order); setView('form'); }}>
                    <div><div className="font-bold text-lg">{order.brand} {order.model} <span className="text-gray-400 text-sm">#{order.orderNumber}</span></div><div className="text-sm text-gray-500">{order.clientName} • {order.plates}</div></div>
                    <div className="text-right"><div className={`text-xs px-2 py-1 rounded-full ${order.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{order.status === 'active' ? 'En Taller' : 'Entregado'}</div></div>
                  </div>
              ))}</div>
            }
          </div>
