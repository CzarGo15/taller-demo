...
if (typeof __firebase_config !== 'undefined') {
  ...
  // Entorno Producción (GitHub / Local) - TUS LLAVES
  firebaseConfig = {
...
export default function App() {
  ...
  const [authError, setAuthError] = useState<string | null>(null); // Nuevo estado para errores
    ...
    const initAuth = async () => {
      try {
        ...
        if (msg.includes("auth/operation-not-allowed")) {
          msg = "Debes habilitar 'Anonymous' en Firebase Authentication > Sign-in method.";
        } else if (msg.includes("auth/unauthorized-domain")) {
          msg = "Este dominio no está autorizado. Agrégalo en Firebase Authentication > Settings > Authorized Domains.";
        }
        setAuthError(msg);
      }
    };
  ...
  if (authError) {
    return (
      <div className="flex flex-col items-center justify-center h-screen p-8 text-center bg-gray-100">
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-md">
          <div className="text-red-600 font-bold text-xl mb-4 flex items-center justify-center gap-2">
            <Trash2 className="w-6 h-6" /> Error de Conexión
          </div>
          <div className="text-gray-700 mb-4">{authError}</div>
          <div className="text-xs text-gray-500 bg-gray-100 p-2 rounded text-left overflow-auto">
            Ayuda rápida:<br/>
            1. Ve a Firebase Console &gt; Authentication.<br/>
            2. Pestaña "Sign-in method" &gt; Habilita "Anonymous".<br/>
            3. Pestaña "Settings" &gt; "Authorized Domains" &gt; Agrega "czargo15.github.io".
          </div>
        </div>
      </div>
    );
      ...
      {view === 'list' && (
        <div className="p-6 max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={createNewOrder} className="bg-blue-600 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-2 hover:bg-blue-700 transition">
            ...
            <div className="border-2 border-blue-900 rounded-lg p-2 mb-2 flex justify-between items-start">
              <div className="w-1/3">
                 <div className="flex items-center gap-2 text-blue-900 font-black italic text-xl">
                   <Car className="w-8 h-8" />
                   <div><div>MULTISERVICIO</div><div className="text-sm font-normal">AUTOMOTRIZ ARREDONDO</div></div>
                 </div>
                 <div className="text-[10px] mt-2 leading-tight text-gray-600">Quevedo 2708 Col. Puerto México<br/>Coatzacoalcos, Ver.<br/>Tels: (921) 21 3 77 98 / 921 569 6614<br/>servicioarredondo@prodigy.net.mx</div>
              </div>
              <div className="w-1/3 text-center pt-2">
                ...
                <div className="text-[9px] mt-1 text-gray-500">Horario: L-V 8:30 a 18:30<br/>Sáb 8:30 a 14:00</div>
              </div>
            ...
            <div className="mb-2">
              ...
              <div className="grid grid-cols-5 gap-0 border border-gray-300 text-[9px] print:text-[8px]">
                ...
                <div className="col-span-2 border-r border-gray-300">
                   {INVENTORY_GROUPS.interiores.map(item => (<InventoryItem key={item} label={item} value={currentOrder.inventory[item]} onChange={(val:any) => setCurrentOrder({...currentOrder, inventory: {...currentOrder.inventory, [item]: val}})} />))}
                </div>
                <div className="col-span-2 border-r border-gray-300">
                   {INVENTORY_GROUPS.motor.map(item => (<InventoryItem key={item} label={item} value={currentOrder.inventory[item]} onChange={(val:any) => setCurrentOrder({...currentOrder, inventory: {...currentOrder.inventory, [item]: val}})} />))}
                   <div className="bg-gray-100 font-bold p-1 border-t border-b">LLANTAS</div>
                   {INVENTORY_GROUPS.llantas.map(item => (<InventoryItem key={item} label={item} value={currentOrder.inventory[item]} onChange={(val:any) => setCurrentOrder({...currentOrder, inventory: {...currentOrder.inventory, [item]: val}})} />))}
                </div>
                <div className="col-span-1">
                   {INVENTORY_GROUPS.exterior.map(item => (<div className="flex justify-between items-center px-1 border-b border-gray-100 h-5" key={item}><span className="truncate">{item}</span><SimpleStateToggle value={currentOrder.inventory[item]} onChange={(val:any) => setCurrentOrder({...currentOrder, inventory: {...currentOrder.inventory, [item]: val}})}/></div>))}
                   <div className="bg-gray-100 font-bold p-1 border-t border-b">CAJUELA</div>
                   {INVENTORY_GROUPS.cajuela.map(item => (<div className="flex justify-between items-center px-1 border-b border-gray-100 h-5" key={item}><span className="truncate">{item}</span><SimpleStateToggle value={currentOrder.inventory[item]} onChange={(val:any) => setCurrentOrder({...currentOrder, inventory: {...currentOrder.inventory, [item]: val}})}/></div>))}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 border-t-2 border-gray-800 pt-4 text-center">
               <div className="w-full max-w-sm mx-auto">
                 <div className="h-24 border-b border-gray-400 mb-1 flex items-end justify-center relative">
                   ...
                   ) : (<SignaturePad onSave={(signature: string) => setCurrentOrder({...currentOrder, clientSignature: signature})} />)}
                 </div>
